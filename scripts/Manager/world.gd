...
#  SPAWN PROJECTILES
# ─────────────────────────────────────────
# Point d'entrée appelé par le joueur LOCAL
func spawn_projectile(scene: PackedScene, spawn_pos: Vector2, dir: Vector2, owner_peer_id: int) -> void:
	if not multiplayer.has_multiplayer_peer():
		# Solo
		_do_spawn_projectile(scene.resource_path, spawn_pos, dir, owner_peer_id)
	elif multiplayer.is_server():
		# Hôte : spawn local + broadcast aux clients
		_do_spawn_projectile(scene.resource_path, spawn_pos, dir, owner_peer_id)
		_rpc_spawn_projectile.rpc(scene.resource_path, spawn_pos, dir, owner_peer_id)
	else:
		# Client : demander au serveur via RPC
		_rpc_request_projectile.rpc_id(1, scene.resource_path, spawn_pos, dir, owner_peer_id)

# Client → Serveur : demande de spawn
@rpc("any_peer", "call_remote", "reliable")
func _rpc_request_projectile(scene_path: String, spawn_pos: Vector2, dir: Vector2, owner_peer_id: int) -> void:
	# Spawn côté serveur
	_do_spawn_projectile(scene_path, spawn_pos, dir, owner_peer_id)
	# Broadcast à tous les clients
	_rpc_spawn_projectile.rpc(scene_path, spawn_pos, dir, owner_peer_id)

# Serveur → Tous les clients
@rpc("authority", "call_remote", "reliable")
func _rpc_spawn_projectile(scene_path: String, spawn_pos: Vector2, dir: Vector2, owner_peer_id: int) -> void:
	_do_spawn_projectile(scene_path, spawn_pos, dir, owner_peer_id)

# Spawn effectif, exécuté sur chaque machine
func _do_spawn_projectile(scene_path: String, spawn_pos: Vector2, dir: Vector2, owner_peer_id: int) -> void:
	...
	projectile.owner_peer_id = owner_peer_id  # IMPORTANT : assigne le peer_id du propriétaire
