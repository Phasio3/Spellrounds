...
func cast_spell(index: int) -> void:
	...
	if spell.has("cast_func"):
		spell["cast_func"].call(spell["scene"])
...
func cast_projectile(spell_scene) -> void:
	...
	if world and world.has_method("spawn_projectile"):
		var peer_id = get_multiplayer_authority()  # ou multiplayer.get_unique_id() si c'est le joueur local
		world.spawn_projectile(spell_scene, spawn_pos, dir, peer_id)
	else:
